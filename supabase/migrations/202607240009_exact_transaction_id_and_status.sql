drop function public.read_inventory_agent_transactions(uuid, text, integer);

create function public.read_inventory_agent_transactions(
...
as $$
  with query_input as (
    select
      nullif(trim(p_query), '') as normalized_query,
      case
        when trim(coalesce(p_query, '')) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
          then lower(trim(p_query))
        else null
      end as exact_transaction_id
  ),
  summaries as (
    select
      transaction.id,
      transaction.transaction_type::text as kind,
      transaction.status::text as lifecycle_status,
  ...
  where query_input.normalized_query is null
    or (
      query_input.exact_transaction_id is not null
      and ranked.id::text = query_input.exact_transaction_id
    )
    or (
      query_input.exact_transaction_id is null
      and ranked.matched_terms > 0
      and (
  ...
  order by
    case
      when query_input.exact_transaction_id is not null
        and ranked.id::text = query_input.exact_transaction_id then 0
      else 1
    end,
    case when query_input.normalized_query is not null then ranked.matched_terms end desc,
    ranked.applied_at desc,
    ranked.id desc
  limit least(greatest(coalesce(p_limit, 10), 1), 20);
$$;

revoke all on function public.read_inventory_agent_transactions(
  uuid, text, integer
) from public, anon, authenticated;
grant execute on function public.read_inventory_agent_transactions(
  uuid, text, integer
) to service_role;

comment on function public.read_inventory_agent_transactions(uuid, text, integer) is
  'Returns exact UUID matches or ranked transaction search results with stored type and status.';
